...
class GeminiLogoDetector:
    ...
    async def process_video_segment(self, video_path: str, start_time: float = 0, duration: float = 7) -> Optional[bytes]:
        ...
        # Use XVID codec which is more widely supported on Windows
        fourcc = cv2.VideoWriter_fourcc(*'XVID')  # Change from avc1 to XVID
        output_filename = f"processed_segment_{int(time.time())}_{random.randint(1000,9999)}.avi"
